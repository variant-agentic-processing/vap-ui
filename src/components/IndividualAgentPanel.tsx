...
interface IndividualAgentPanelProps {
  individualId: string;
  sample: Sample | null;
}
...
function buildContext(individualId: string, sample: Sample | null): string {
  const parts = [
    `The user is viewing individual ${individualId}.`,
  ];
  if (sample) {
...
export function IndividualAgentPanel({ individualId, sample }: IndividualAgentPanelProps) {
  ...
  function handleSubmit() {
    ...
    setInput("");
    ask(q, buildContext(individualId, sample));
  }
      ...
      {isOpen && (
        <div className="fixed bottom-0 right-0 z-40 flex h-[600px] w-[420px] flex-col rounded-tl-2xl border-l border-t border-brand-border bg-brand-navy shadow-2xl">
          ...
          <div className="flex items-center justify-between border-b border-brand-border px-4 py-3">
            <div>
              <p className="text-xs font-semibold text-brand-cyan font-mono">{individualId}</p>
              <p className="text-xs text-brand-muted">Ask about this individual</p>
            </div>
              ...
              emptyState={
                <div className="flex h-full items-center justify-center text-center px-4">
                  <p className="text-sm text-brand-muted">
                    ...
                    <span className="font-mono text-brand-text">{individualId}</span>
          ...
          <div className="border-t border-brand-border">
            <QueryInput
              ...
              placeholder={`Ask about ${individualId}…`}
            />
          </div>
        </div>
      )}
